...
function display_monthcalendar($agendaitems, $month, $year, $weekdaynames, $monthName, $langToday) {
	...
	$backwardsURL = "".esc($_SERVER[PHP_SELF])."?month=".($month==1 ? 12 : $month-1)."&year=".($month==1 ? $year-1 : $year);
	$forewardsURL = "".esc($_SERVER[PHP_SELF])."?month=".($month==12 ? 1 : $month+1)."&year=".($month==12 ? $year+1 : $year);
