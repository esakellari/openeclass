...
if (isset($a)) {
	switch ($a) {
		case 1:
			$navigation[] = array("url" => "".esc($_SERVER['PHP_SELF'])."", "name" => $langListFaculteActions);
...
******************************************************************************/
	...
	<ul id='opslist'>
	<li><a href='".esc($_SERVER['PHP_SELF'])."?a=1'>".$langAdd."</a></li>
	</ul>
	</div>";
...
if (!isset($a)) {
	...
	for ($j = 0; $j < mysql_num_rows($sql); $j++) {
		...
		<a href='".esc($_SERVER['PHP_SELF'])."?a=2&c=".$logs['id']."'>
		<img src='../../images/delete.gif' border='0' title='$langDelete'></img></a>&nbsp;&nbsp;
		<a href='".esc($_SERVER['PHP_SELF'])."?a=3&c=".$logs['id']."'>
...
elseif ($a == 1)  {
	if (isset($add)) {
	$codefaculte = esc($codefaculte);
	$faculte = esc($faculte);
		// Check for empty fields
		if (empty($codefaculte) or empty($faculte)) {
			$tool_content .= "<p>".$langEmptyFaculte."</p><br />";
			$tool_content .= "<center><p>
			<a href=\"".esc($_SERVER['PHP_SELF'])."?a=1\">".$langReturnToAddFaculte."</a></p></center>";
			}
		// Check for greek letters
		elseif (!preg_match("/^[A-Z0-9a-z_-]+$/", $codefaculte)) {
			$tool_content .= "<p>".$langGreekCode."</p><br />";
			$tool_content .= "<center><p><a href=\"".esc($_SERVER['PHP_SELF'])."?a=1\">".$langReturnToAddFaculte."</a></p></center>";
			}
		...
		$tool_content .= "<form method=\"post\" action=\"".esc($_SERVER['PHP_SELF'])."?a=1\">";
	...
	$tool_content .= "<br /><p align='right'><a href='".esc($_SERVER['PHP_SELF'])."'>".$langBack."</a></p>";
}
// Delete faculty
elseif ($a == 2) {
        $c = intval($_GET['c']);
...
elseif ($a == 3)  {
	...
	if (isset($_POST['edit'])) {
		...
		if (empty($faculte)) {
			$tool_content .= "<p>".$langEmptyFaculte."</p><br>";
			$tool_content .= "<p align='right'><a href='".esc($_SERVER['PHP_SELF'])."?a=3&c=$c'>$langReturnToEditFaculte</a></p>";
			}
		...
		$tool_content .= "<form method='post' action='".esc($_SERVER['PHP_SELF'])."?a=3'>";
		$tool_content .= "<table width='99%' class='FormData'>
		<tbody>
		<tr>
		<th width='220'>&nbsp;</th>
