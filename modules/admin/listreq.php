...
$basetoolurl = esc($_SERVER['PHP_SELF']);
 ...
 // Display Actions Toolbar
  $tool_content .= "
	  ...
	  <li><a href='newuseradmin.php$linkget'>$linkreg</a></li>
          <li><a href='".esc($_SERVER['PHP_SELF'])."?show=closed$reqtype'>$langReqHaveClosed</a></li>
          <li><a href='".esc($_SERVER['PHP_SELF'])."?show=rejected$reqtype'>$langReqHaveBlocked</a></li>
          <li><a href='".esc($_SERVER['PHP_SELF'])."?show=accepted$reqtype'>$langReqHaveFinished</a></li>
        </ul>
      </div>";
		...
		while ($req = mysql_fetch_array($sql)) {
			...
			$tool_content .= "<td align=\"center\">
				...
				<small>".nice_format(date("Y-m-d", strtotime($req['date_closed'])))."</small></td>";
                	$tool_content .= "<td>".$req['comment']."</td>";
			$tool_content .= "<td align=center>
			<a href='".esc($_SERVER['PHP_SELF'])."?id=$req[rid]&show=closed$reqtype'>$langRestore</a>
			</td></tr>";
			$k++;
		}
	}
...
else
{
	...
	while ($req = mysql_fetch_array($sql)) {
		...
		$tool_content .= "<td align='center'>
		<a href='".esc($_SERVER['PHP_SELF'])."?id=$req[rid]&amp;close=1$reqtype' onclick='return confirmation();'>$langClose</a><br />
		<a href='".esc($_SERVER['PHP_SELF'])."?id=$req[rid]&amp;close=2$reqtype'>$langRejectRequest</a>";
...
if (!empty($show)) {
	$tool_content .= "<p align=\"right\"><a href=\"".esc($_SERVER['PHP_SELF'])."\">$langBackRequests</a></p><br>";
}
